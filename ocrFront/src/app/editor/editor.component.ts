...
import {Label} from "../category/category.component";
...
export class EditorComponent {
  ...
  getImage() {
    return this.http.get(`http://localhost:5000/getImage/${this.id}`, { responseType: 'blob' });
  }
  ...
  loadImage() {
    this.getImage().subscribe(response => {
      ...
      this.setImageContainerDimensions(objectURL);
      this.setImageContainerBackground(objectURL);
    });
  }
  ...
  @ViewChild('drawingField') drawingField!: ElementRef;
  ...
  onFileChange(event: any) {
    ...
    if (file) {
      ...
      reader.onload = (e: any) => {
        ...
        this.setImageContainerBackground(imgSrc);
        this.setImageContainerDimensions(imgSrc);
      };
      reader.readAsDataURL(file);
    }
  }

  setImageContainerBackground(src: string) {
    // if (this.drawingField) {
      const container = this.drawingField.nativeElement;
      container.style.backgroundImage = `url(${src})`;
      container.style.backgroundSize = 'cover'; // Optional, to cover the container fully
      container.style.backgroundPosition = 'center'; // Optional, to center the image
    // }
  }

  setImageContainerDimensions(src: string) {
    // if (this.drawingField) {
      const img = new Image();
      img.src = src;
  ...
  draw_shape() {
    if (this.isrectangle) {
      ...
      let field = prompt('please enter the field label', '');
      console.log(field)
      this.count += 1;
      let rect: Rect;
      rect = new Konva.Rect({
  ...
  async sendShapesToSave() {
    ...
    labels = shaperec.map((shape) => {
      let label = new Label()
      label.name = shape.attrs.label
      label.top_x = shape.x()
      label.top_y = shape.y()
      label.bottom_x = shape.width() + shape.x()
      label.bottom_y = shape.height() + shape.y()
      return label
    });
    console.log(labels)
    this.http.post('http://localhost:8080/api/type/addLabel/'+this.id, labels).subscribe((response) => {
